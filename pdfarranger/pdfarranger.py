...
class PdfArranger(Gtk.Application):
    ...
    def split_pages(self, _action, _parameter, _unknown):
        ...
        # Before summoning the splitter dialog, check whether we have contiguous pages
        # If the pages are contiguous, the user may choose to "unimpose" from the splitter dialog
        selected_page_numbers = [ x.get_indices()[0] for x in selection ]
        unimposable = True
        if len(selected_page_numbers) > 1:
            previous_page = selected_page_numbers[0]
            for page in selected_page_numbers[1:]:
                if page != previous_page + 1:
                    print("The page selection is not contiguous, so unimposing is made unavailable: " + str(selected_page_numbers))
                    unimposable = False
                previous_page = page

        diag = splitter.Dialog(self.window, unimposable)
        leftcrops, topcrops, unimpose = diag.run_get()
        ...
        with self.render_lock():
            if unimpose:
                ...
                # to operate on selection, so any selection of pages will be supported.
                ...
                # Sanity check
                assert None not in reorder_pages
