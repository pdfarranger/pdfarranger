...
class PaperSizeWidget(Gtk.Grid):
    def __init__(self, size, margin=16):
        ...
        paper_list = ['iso_a0', 'iso_a1', 'iso_a2', 'iso_a3', 'iso_a4', 'iso_a5', 'na_letter', 'na_legal', 'na_ledger']
...
class PlaceContentDialog:
    ...
    def __init__(self, window, selection, model, pdfqueue, callback):
        ...
        initial_sx = (pt.shift_x if pt else 0.0) * MM_PER_PT
        initial_sy = (pt.shift_y if pt else 0.0) * MM_PER_PT
        initial_sc = pt.scale if pt else 1.0
        initial_sp = -(pt.spin_deg if pt else 0.0)  # stored CCW, displayed CW
        ...
        grid.attach(Gtk.Label(label=_("Shift X (mm):"), xalign=1), 0, 0, 1, 1)
        self.shift_x = Gtk.SpinButton.new_with_range(-500, 500, 1)
        self.shift_x.set_digits(1)
        ...
        grid.attach(Gtk.Label(label=_("Shift Y (mm):"), xalign=1), 0, 1, 1, 1)
        self.shift_y = Gtk.SpinButton.new_with_range(-500, 500, 1)
        self.shift_y.set_digits(1)
        ...
        self.spin = Gtk.SpinButton.new_with_range(-360, 360, 0.1)
    ...
    def _on_mouse_motion(self, widget, event):
        if self._dragging:
            ...
            self.shift_x.set_value(self._start_shift_x + (dx / zoom) * MM_PER_PT)
            self.shift_y.set_value(self._start_shift_y - (dy / zoom) * MM_PER_PT)
            return True
        return False
    ...
    def _build_transform(self):
        sx = self.shift_x.get_value() * PT_PER_MM
        sy = self.shift_y.get_value() * PT_PER_MM
        sc = self.scale.get_value()
        sp = -self.spin.get_value()  # UI is CW-positive, stored as CCW-positive
